...
const makeTrayMenu = (opts: { server?: string } = {}) => {
...
app.on("ready", async () => {
  ...
  await server.start({ port, public: path.join(__dirname, "web") });
  tray.setContextMenu(makeTrayMenu({ server: `https://localhost:${port}` }));
});
