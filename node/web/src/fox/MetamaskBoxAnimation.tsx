...
> {
  ...
  private _handleMouseMove = (ev: MouseEvent) => {
    ...
    ev.preventDefault();
    this._handleMove(ev.clientX, ev.clientY);
  };
  ...
  private _handleTouchStart = (ev: TouchEvent) => {
    ...
    if (ev.touches.length === 1) {
      this._handleMouseDown(touch.clientX, touch.clientY);
    } else {
  ...
  public render() {
    ...
    return (
      <div
        ...
        onMouseDown={(ev) => {
          if (MetamaskBoxAnimation.renderer && this.props.allowDragging) {
            this._handleMouseDown(ev.clientX, ev.clientY);
          }
        }}
        onMouseMove={(ev) => {
          if (this.props.followMouse && MetamaskBoxAnimation.renderer) {
            MetamaskBoxAnimation.renderer.foxLookAt(ev.clientX, ev.clientY);
          }
        }}
        ref={(container) => {
